...
def process_video_folder(video_folder, model, platform, co_helper):
    ...
    for video_file in video_files:
        ...
        # Create video writer to save results
        output_path = os.path.join('./result', f'output_{video_file}')
        if not os.path.exists('./result'):
            os.mkdir('./result')
        fourcc = cv2.VideoWriter_fourcc(*'XVID')
        out = cv2.VideoWriter(output_path, fourcc, 30, (IMG_SIZE[0], IMG_SIZE[1]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device_id', type=str, default=None, help='Device ID for the RKNN model')
